...
Pak stačí vytvořit Engine objekt. Ten má 2 hlavní požadavky. Vytvořte ho pod jménem **ge** a jako **globalní staticky alokovanou** proměnou. Konstruktor bere *název*, *šířku* a *výšku* okna. Čtvrtý volitelný parameter je nastavení enginu, ale o tom později.
...
Tento CMakeLists.txt není nijak zvláštní, nastaví standardy, připojí se složka **graphicengine** (*která má svůj vlastní CMakeLists.txt*), přidá se **main.cpp** ke kompilaci do spustitelného souboru a napojí se knihovna (název přesně *graphicengine*) se napojí na projekt.

Co je ale lehce nestandartní je funkce graphicengine_setup(). Je to funkce nadefinovaná v CMakeLists.txt ve složce graphicengine a dělá to že **POST_BUILD** daného targetu (${PROJECT_NAME}) to ve výsledné složce překopíruje zdroje enginu. Je to složka *engine* ve které jsou například shadery enginu.
...
Engine má i nějaké nastavení, kterými můžete uzpůsobit engine vašim potřebám.
...
Člověk si vytvoří vlastní instanci EngineSettings **struct** u a pomocí volitelných parametrů dokáže upravit právě jenom **gamma_correction** nastavení, aniž by měnil ostatní.
